...
This chapter defines the boundary between `bic` link metadata and downstream build-system work.

That boundary matters because `bic` is intentionally a library for analysis and normalized link
intent, not a replacement for a platform linker, package manager, or full build graph.

## What `bic` Resolves Today

`bic` currently resolves and preserves:
...
## What `bic` Does Not Resolve Today

`bic` does not currently promise:
...
If `bic` claimed to fully resolve all of that itself, it would either become much less general or
...
4. optionally compare the resolved artifacts with `bic` symbol inventories and validation reports
...
It may rely on `bic` for extraction, normalization, and evidence gathering, while still keeping
