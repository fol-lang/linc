...
fn difficult_record_evidence_reports_field_offsets_for_nested_non_bitfield_record() {
    eprintln!("RUN: host C compiler nested-record layout evidence");
    let header = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("tests/fixtures/difficult_record_evidence.h");
    ...
    assert_eq!(
        ...
        Some(AbiConfidence::FieldOffsetsProbed)
    );
...
fn difficult_record_evidence_preserves_partial_bitfield_signal_for_nested_record_family() {
    eprintln!("RUN: host C compiler nested-bitfield layout evidence");
    let header = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("tests/fixtures/difficult_record_evidence.h");
    let result = common::process(
        &linc::raw_headers::HeaderConfig::new()
